<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>

<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<c:set var="pageTitle" value="MAIN"></c:set>
<link rel="stylesheet" href="/resource/main.css" />
<link rel="stylesheet" href="/resource/common.css" />

<%@ include file="../common/head1300.jspf"%>


<div class="section" id="ranking">
	<div class="list">
		<div class="item-heading-4-team-ranking">구단 순위</div>
	</div>
	<div class="list-item">
		<div class="table">
			<div class="header-row">
				<div class="cell">
					<div class="div2">순위</div>
				</div>
				<div class="cell2">
					<div class="div2">팀명</div>
				</div>
				<div class="cell3">
					<div class="div2">경기</div>
				</div>
				<div class="cell4">
					<div class="div3">승</div>
				</div>
				<div class="cell5">
					<div class="div3">패</div>
				</div>
				<div class="cell6">
					<div class="div4">무</div>
				</div>
				<div class="cell7">
					<div class="div5">승률</div>
				</div>
				<div class="cell8">
					<div class="div6">게임차</div>
				</div>
				<div class="cell9">
					<div class="div2">연속</div>
				</div>
			</div>
			<div class="body">
				<div class="row">
					<div class="cell10">
						<div class="_12">1</div>
					</div>
					<div class="data">
						<div class="kia">KIA</div>
					</div>
					<div class="data2">
						<div class="_124">124</div>
					</div>
					<div class="data3">
						<div class="_73">73</div>
					</div>
					<div class="data4">
						<div class="_49">49</div>
					</div>
					<div class="data5">
						<div class="_2">2</div>
					</div>
					<div class="data6">
						<div class="_0-598">0.598</div>
					</div>
					<div class="data7">
						<div class="div7">-</div>
					</div>
					<div class="data8">
						<div class="_13">1패</div>
					</div>
				</div>
				<div class="row2">
					<div class="cell11">
						<div class="_22">2</div>
					</div>
					<div class="data9">
						<div class="div8">삼성</div>
					</div>
					<div class="data10">
						<div class="_125">125</div>
					</div>
					<div class="data11">
						<div class="_69">69</div>
					</div>
					<div class="data12">
						<div class="_54">54</div>
					</div>
					<div class="data13">
						<div class="_23">2</div>
					</div>
					<div class="data14">
						<div class="_0-561">0.561</div>
					</div>
					<div class="data15">
						<div class="_4-5">4.5</div>
					</div>
					<div class="data16">
						<div class="_4">4승</div>
					</div>
				</div>
				<div class="row3">
					<div class="cell11">
						<div class="_3">3</div>
					</div>
					<div class="data9">
						<div class="lg">LG</div>
					</div>
					<div class="data10">
						<div class="_123">123</div>
					</div>
					<div class="data11">
						<div class="_64">64</div>
					</div>
					<div class="data12">
						<div class="_57">57</div>
					</div>
					<div class="data13">
						<div class="_23">2</div>
					</div>
					<div class="data14">
						<div class="_0-529">0.529</div>
					</div>
					<div class="data15">
						<div class="_8-5">8.5</div>
					</div>
					<div class="data16">
						<div class="_24">2패</div>
					</div>
				</div>
				<div class="row4">
					<div class="cell11">
						<div class="_42">4</div>
					</div>
					<div class="data9">
						<div class="div8">두산</div>
					</div>
					<div class="data10">
						<div class="_127">127</div>
					</div>
					<div class="data11">
						<div class="_64">64</div>
					</div>
					<div class="data12">
						<div class="_61">61</div>
					</div>
					<div class="data13">
						<div class="_23">2</div>
					</div>
					<div class="data14">
						<div class="_0-512">0.512</div>
					</div>
					<div class="data15">
						<div class="_10-5">10.5</div>
					</div>
					<div class="data16">
						<div class="_14">1패</div>
					</div>
				</div>
				<div class="row5">
					<div class="cell11">
						<div class="_5">5</div>
					</div>
					<div class="data9">
						<div class="kt">KT</div>
					</div>
					<div class="data10">
						<div class="_125">125</div>
					</div>
					<div class="data11">
						<div class="_61">61</div>
					</div>
					<div class="data12">
						<div class="_62">62</div>
					</div>
					<div class="data13">
						<div class="_23">2</div>
					</div>
					<div class="data14">
						<div class="_0-496">0.496</div>
					</div>
					<div class="data15">
						<div class="_12-5">12.5</div>
					</div>
					<div class="data16">
						<div class="_24">2승</div>
					</div>
				</div>
				<div class="row6">
					<div class="cell11">
						<div class="_6">6</div>
					</div>
					<div class="data9">
						<div class="ssg">SSG</div>
					</div>
					<div class="data10">
						<div class="_1242">124</div>
					</div>
					<div class="data11">
						<div class="_59">59</div>
					</div>
					<div class="data12">
						<div class="_64">64</div>
					</div>
					<div class="data13">
						<div class="_15">1</div>
					</div>
					<div class="data14">
						<div class="_0-480">0.480</div>
					</div>
					<div class="data15">
						<div class="_14-5">14.5</div>
					</div>
					<div class="data16">
						<div class="_14">1승</div>
					</div>
				</div>
				<div class="row7">
					<div class="cell11">
						<div class="_7">7</div>
					</div>
					<div class="data9">
						<div class="div8">한화</div>
					</div>
					<div class="data10">
						<div class="_121">121</div>
					</div>
					<div class="data11">
						<div class="_57">57</div>
					</div>
					<div class="data12">
						<div class="_62">62</div>
					</div>
					<div class="data13">
						<div class="_23">2</div>
					</div>
					<div class="data14">
						<div class="_0-479">0.479</div>
					</div>
					<div class="data15">
						<div class="_14-5">14.5</div>
					</div>
					<div class="data16">
						<div class="_14">1패</div>
					</div>
				</div>
				<div class="row8">
					<div class="cell11">
						<div class="_8">8</div>
					</div>
					<div class="data9">
						<div class="div8">롯데</div>
					</div>
					<div class="data10">
						<div class="_118">118</div>
					</div>
					<div class="data11">
						<div class="_53">53</div>
					</div>
					<div class="data12">
						<div class="_62">62</div>
					</div>
					<div class="data13">
						<div class="_32">3</div>
					</div>
					<div class="data14">
						<div class="_0-461">0.461</div>
					</div>
					<div class="data15">
						<div class="_16-5">16.5</div>
					</div>
					<div class="data16">
						<div class="_14">1승</div>
					</div>
				</div>
				<div class="row9">
					<div class="cell11">
						<div class="_9">9</div>
					</div>
					<div class="data9">
						<div class="nc">NC</div>
					</div>
					<div class="data10">
						<div class="_120">120</div>
					</div>
					<div class="data11">
						<div class="_53">53</div>
					</div>
					<div class="data12">
						<div class="_65">65</div>
					</div>
					<div class="data13">
						<div class="_23">2</div>
					</div>
					<div class="data14">
						<div class="_0-449">0.449</div>
					</div>
					<div class="data15">
						<div class="_18">18</div>
					</div>
					<div class="data16">
						<div class="_14">1승</div>
					</div>
				</div>
				<div class="row10">
					<div class="cell11">
						<div class="_10">10</div>
					</div>
					<div class="data9">
						<div class="div8">키움</div>
					</div>
					<div class="data10">
						<div class="_123">123</div>
					</div>
					<div class="data11">
						<div class="_53">53</div>
					</div>
					<div class="data12">
						<div class="_70">70</div>
					</div>
					<div class="data13">
						<div class="_0">0</div>
					</div>
					<div class="data14">
						<div class="_0-431">0.431</div>
					</div>
					<div class="data15">
						<div class="_20-5">20.5</div>
					</div>
					<div class="data16">
						<div class="_33">3패</div>
					</div>
				</div>
			</div>
		</div>
		<div class="_2024-8-29">2024년 8월 29일 기준</div>
	</div>
	<div class="section2" id="article">
		<div class="kbo-bar">
			<div class="kbo">KBO 소식</div>
			<svg class="link" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
			<path fill="currentColor" d="M19 12.998h-6v6h-2v-6H5v-2h6v-6h2v6h6z" /></svg>
		</div>
		<div class="kbo2">
			<div class="_1-kbo">
				<div class="link2">
					<div class="label">
						<div class="div9">보도</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="kbo-1">KBO 소식 1</div>
					</div>
				</div>
			</div>
			<div class="_2-kbo">
				<div class="link2">
					<div class="label">
						<div class="div9">보도</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="kbo-12">KBO 소식 1</div>
					</div>
				</div>
			</div>
			<div class="_3-kbo">
				<div class="link2">
					<div class="label">
						<div class="div9">보도</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="kbo-3">KBO 소식 3</div>
					</div>
				</div>
			</div>
			<div class="_4-kbo">
				<div class="link2">
					<div class="label">
						<div class="div9">보도</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="kbo-4">KBO 소식 4</div>
					</div>
				</div>
			</div>
			<div class="_5-kbo">
				<div class="link2">
					<div class="label">
						<div class="div9">보도</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_52">KBO 소식 5</div>
					</div>
				</div>
			</div>
		</div>
		<div class="bar">
			<div class="div10">전체글 보기</div>
			<svg class="link3" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
			<path fill="currentColor" d="M19 12.998h-6v6h-2v-6H5v-2h6v-6h2v6h6z" /></svg>
		</div>
		<div class="div11">
			<div class="_16">
				<div class="link2">
					<div class="label">
						<div class="div12">공지</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_17">최신글 1</div>
					</div>
				</div>
			</div>
			<div class="_25">
				<div class="link2">
					<div class="label">
						<div class="div9">자유</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_26">최신글 2</div>
					</div>
				</div>
			</div>
			<div class="_34">
				<div class="link2">
					<div class="label">
						<div class="div9">후기</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_35">최신글 3</div>
					</div>
				</div>
			</div>
			<div class="_43">
				<div class="link2">
					<div class="label">
						<div class="div9">자유</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_44">최신글 4</div>
					</div>
				</div>
			</div>
			<div class="_55">
				<div class="link2">
					<div class="label">
						<div class="div9">자유</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_56">최신글 5</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="section3" id="news">
		<svg class="link4" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
			<path fill="currentColor" d="M19 12.998h-6v6h-2v-6H5v-2h6v-6h2v6h6z" /></svg>
		<div class="div13">
			<div class="_1-kbo">
				<div class="div14">
					<div class="div15">
						<div class="div16">두산</div>
					</div>
					<div class="div17">
						<div class="div16">롯데</div>
					</div>
					<div class="div18">
						<div class="div16">기아</div>
					</div>
					<div class="div19">
						<div class="div16">삼성</div>
					</div>
					<div class="div20">
						<div class="div16">한화</div>
					</div>
				</div>
				<div class="link2">
					<div class="container">
						<div class="_19">최근 네이버 기사 1</div>
					</div>
				</div>
			</div>
			<div class="_110">
				<div class="link2"></div>
				<div class="link2">
					<div class="container">
						<div class="_27">최근 네이버 기사 2</div>
					</div>
				</div>
			</div>
			<div class="_36">
				<div class="link2"></div>
				<div class="link2">
					<div class="container">
						<div class="_37">최근 네이버 기사 3</div>
					</div>
				</div>
			</div>
			<div class="_45">
				<div class="link2"></div>
				<div class="link2">
					<div class="container">
						<div class="_46">최근 네이버 기사 4</div>
					</div>
				</div>
			</div>
			<div class="_58">
				<div class="link2"></div>
				<div class="link2">
					<div class="container">
						<div class="_52">최근 네이버 기사 5</div>
					</div>
				</div>
			</div>
		</div>
		<div class="bar2">
			<div class="div21">최근 기사</div>
		</div>
	</div>
</div>
<div class="section4" id="game">
	<div class="container2">
		<div class="list2">
			<div class="item">
				<div class="border">
					<div class="background">
						<div class="div22">경기예정</div>
					</div>
					<img class="emblem emblem-lg" src="/images/teamLogos/lg.png" />
					<div class="vs">VS</div>
					<img class="emblem emblem-kt" src="/images/teamLogos/kt.png" />
					<div class="list3">
						<div class="item2" style="background: url(item1.png) top left; background-size: 500%; background-repeat: repeat;">
							<div class="link5">
								<div class="div23">예매</div>
							</div>
						</div>
					</div>
					<div class="list4">
						<div class="item3">
							<div class="game-location">수원</div>
							<div class="vertical-divider"></div>
						</div>
						<div class="item4">
							<svg class="sun" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 256 256">
								<path fill="currentColor"
									d="M122 40V16a6 6 0 0 1 12 0v24a6 6 0 0 1-12 0m68 88a62 62 0 1 1-62-62a62.07 62.07 0 0 1 62 62m-12 0a50 50 0 1 0-50 50a50.06 50.06 0 0 0 50-50M59.76 68.24a6 6 0 1 0 8.48-8.48l-16-16a6 6 0 0 0-8.48 8.48Zm0 119.52l-16 16a6 6 0 1 0 8.48 8.48l16-16a6 6 0 1 0-8.48-8.48M192 70a6 6 0 0 0 4.24-1.76l16-16a6 6 0 0 0-8.48-8.48l-16 16A6 6 0 0 0 192 70m4.24 117.76a6 6 0 0 0-8.48 8.48l16 16a6 6 0 0 0 8.48-8.48ZM46 128a6 6 0 0 0-6-6H16a6 6 0 0 0 0 12h24a6 6 0 0 0 6-6m82 82a6 6 0 0 0-6 6v24a6 6 0 0 0 12 0v-24a6 6 0 0 0-6-6m112-88h-24a6 6 0 0 0 0 12h24a6 6 0 0 0 0-12" /></svg>
							<div class="vertical-divider"></div>
						</div>
						<div class="item-18-30">18:30</div>
					</div>
					<div class="item5" style="background: url(item4.png) top left; background-size: 500%; background-repeat: repeat;">
						<div class="link5">
							<div class="div25">구장 정보</div>
						</div>
					</div>
				</div>
			</div>
			<div class="item6">
				<div class="border">
					<div class="background">
						<div class="div22">경기예정</div>
					</div>
					<img class="emblem emblem-lotte" src="/images/teamLogos/lotte.png" />
					<div class="vs">VS</div>
					<img class="emblem emblem-kiwoom" src="/images/teamLogos/kiwoom.png" />
					<div class="list5">
						<div class="list6">
							<div class="item2" style="background: url(item6.png) top left; background-size: 500%; background-repeat: repeat;">
								<div class="link5">
									<div class="div23">예매</div>
								</div>
							</div>
						</div>
						<div class="item7" style="background: url(item7.png) top left; background-size: 500%; background-repeat: repeat;">
							<div class="link5">
								<div class="div25">구장 정보</div>
							</div>
						</div>
					</div>
					<div class="list4">
						<div class="item3">
							<div class="game-location">고척</div>
							<div class="vertical-divider"></div>
						</div>
						<div class="item4">
							<svg class="sun" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 256 256">
								<path fill="currentColor"
									d="M122 40V16a6 6 0 0 1 12 0v24a6 6 0 0 1-12 0m68 88a62 62 0 1 1-62-62a62.07 62.07 0 0 1 62 62m-12 0a50 50 0 1 0-50 50a50.06 50.06 0 0 0 50-50M59.76 68.24a6 6 0 1 0 8.48-8.48l-16-16a6 6 0 0 0-8.48 8.48Zm0 119.52l-16 16a6 6 0 1 0 8.48 8.48l16-16a6 6 0 1 0-8.48-8.48M192 70a6 6 0 0 0 4.24-1.76l16-16a6 6 0 0 0-8.48-8.48l-16 16A6 6 0 0 0 192 70m4.24 117.76a6 6 0 0 0-8.48 8.48l16 16a6 6 0 0 0 8.48-8.48ZM46 128a6 6 0 0 0-6-6H16a6 6 0 0 0 0 12h24a6 6 0 0 0 6-6m82 82a6 6 0 0 0-6 6v24a6 6 0 0 0 12 0v-24a6 6 0 0 0-6-6m112-88h-24a6 6 0 0 0 0 12h24a6 6 0 0 0 0-12" /></svg>
							<div class="vertical-divider"></div>
						</div>
						<div class="item-18-30">18:30</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="section5">
		<div class="bar3">
			<div class="item8">
				<svg class="btn-prev-png" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20">
					<path fill="currentColor" d="m4 10l9 9l1.4-1.5L7 10l7.4-7.5L13 1z" /></svg>
			</div>
			<div class="item9">2024.08.30(목)</div>
			<div class="item10">
				<svg class="btn-next-png" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20">
					<path fill="currentColor" d="M7 1L5.6 2.5L13 10l-7.4 7.5L7 19l9-9z" /></svg>
			</div>
		</div>
		<svg class="material-symbols-calendar-month" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"
			viewBox="0 0 24 24">
			<path fill="currentColor"
				d="M12 14q-.425 0-.712-.288T11 13t.288-.712T12 12t.713.288T13 13t-.288.713T12 14m-4 0q-.425 0-.712-.288T7 13t.288-.712T8 12t.713.288T9 13t-.288.713T8 14m8 0q-.425 0-.712-.288T15 13t.288-.712T16 12t.713.288T17 13t-.288.713T16 14m-4 4q-.425 0-.712-.288T11 17t.288-.712T12 16t.713.288T13 17t-.288.713T12 18m-4 0q-.425 0-.712-.288T7 17t.288-.712T8 16t.713.288T9 17t-.288.713T8 18m8 0q-.425 0-.712-.288T15 17t.288-.712T16 16t.713.288T17 17t-.288.713T16 18M5 22q-.825 0-1.412-.587T3 20V6q0-.825.588-1.412T5 4h1V2h2v2h8V2h2v2h1q.825 0 1.413.588T21 6v14q0 .825-.587 1.413T19 22zm0-2h14V10H5z" /></svg>
	</div>
</div>



<%@ include file="../common/foot.jspf"%>