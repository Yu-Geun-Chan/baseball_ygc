...
public class Doosan {
	...
	private void extractPlayerDataToCSV(WebDriver driver, WebDriverWait wait) {
		...
		try (CSVWriter writer = new CSVWriter(new FileWriter(file, true))) { // true : append 모드로 열기
			...
			for (WebElement row : rows) {
				...
				if (cells.size() >= 7) {
					...
					System.out.println("Number: " + number + ", Name: " + name + ", Team: " + teamName + ", Position: "
							+ position + ", BirthDate: " + birthDate + ", Height: " + height + ", Weight: " + weight
							+ ", Career: " + career);
				}
			}
		} catch (IOException e) {
