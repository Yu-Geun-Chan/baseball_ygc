...
<div class="map_wrap">
	<div id="map" style="width: 600px; height: 800px; position: absolute; top: 0; right: 0;"></div>
	<ul id="category">
...
<script>
	...
	var marker = null; // 기존 마커를 저장하는 변수

	// 여러 "찾아가기" 링크에 대해 이벤트를 추가
	var links = document.querySelectorAll('.move-to-map');
	links.forEach(function(link) {
		link.addEventListener('click', function(event) {
			event.preventDefault(); // 기본 동작 방지 (새 창 열기 방지)

			// 클릭한 링크의 data-* 속성 값을 가져옴
			var lat = parseFloat(this.getAttribute('data-lat')); // 숫자형으로 변환
			var lng = parseFloat(this.getAttribute('data-lng')); // 숫자형으로 변환
			...
			displayPlaceInfo(place); // 장소 정보 오버레이로 표시

			// 줌 레벨 조정 (필요에 따라)
			map.setLevel(3); // 줌 레벨을 조정 (값이 낮을수록 확대됨)
		});
	});
